...
from epytope.Core.Result import CleavageSitePredictionResult, CleavageFragmentPredictionResult
...
class APSSMCleavageSitePredictor(ACleavageSitePrediction):
    ...
    def predict(self, aa_seq, length=None, **kwargs):
        ...
        :param aa_seq: A single :class:`~epytope.Core.Peptide.Peptide` or `~epytope.Core.Protein.Protein` or a list of
                       :class:`~epytope.Core.Peptide` or :class:`~epytope.Core.Protein.Protein`
        :type aa_seq: list(:class:`~epytope.Core.Peptide.Peptide` or :class:`~epytope.Core.Protein.Protein`)
                      or :class:`~epytope.Core.Peptide`/:class:`~epytope.Core.Protein.Protein`
        ...
        def __load_model(length):
            ...
            return getattr(__import__("epytope.Data.pssms."+self.name+".mat."+model, fromlist=[model]),
                           model)
...
class PCM(APSSMCleavageSitePredictor):
    ...
    def predict(self, peptides, length=None, **kwargs):
        ...
        :param aa_seq: A single :class:`~epytope.Core.Peptide.Peptide`/`~epytope.Core.Protein.Protein` or a list of
                       :class:`~epytope.Core.Peptide`/:class:`~epytope.Core.Protein.Protein`
        :type aa_seq: list(:class:`~epytope.Core.Peptide.Peptide` or :class:`~epytope.Core.Protein.Protein`)
                      or :class:`~epytope.Core.Peptide`/:class:`~epytope.Core.Protein.Protein`
        :param int length: The peptide length of the cleavage site model. If None the default value is used.
        :return: Returns a :class:`~epytope.Core.Result.CleavageSitePredictionResult` object
...
class APSSMCleavageFragmentPredictor(ACleavageFragmentPrediction):
    ...
    def predict(self, peptides,  **kwargs):
        ...
        :return: Returns a :class:`epytope.Core.Result.CleavageFragmentPredictionResult` object
        :rtype: :class:`epytope.Core.Result.CleavageFragmentPredictionResult`
        """
        def __load_model(length):
            allele_model = "%%s_%i"%(self.name, length)
            return getattr(__import__("epytope.Data.pssms."+self.name+".mat."+allele_model, fromlist=[allele_model]),
                           allele_model)
