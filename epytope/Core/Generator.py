...
from epytope.Core.Base import COMPLEMENT
...
def _incorp_insertion(seq, var, transId, pos, offset, isReverse=False):
    """
    Incorporates an insertion into the given :class:`~epytope.Core.Transcript.Transcript` sequence (side effect!).

    :param list(char) seq: :class:`~epytope.Core.Transcript.Transcript` sequence as a list
    :param var: The snp variant to incorporate
    :type var: :class:`~epytope.Core.Variant.Variant`
    :param str transId: The :class:`~epytope.Core.Transcript.Transcript` ID of seq
    :param int offset: The offset which has to be added onto the :class:`~epytope.Core.Transcript.Transcript` position of
                       the :class:`~epytope.Core.Variant.Variant`
...
def _check_for_problematic_variants(vars):
    ...
    :rtype: bool
    :invariant: list(:class:`~epytope.Core.Variant.Variant`) vars: List is sorted based on genome position in descending
                                                                 order
    """
    def get_range(var):
        current_range = [0, 0]
...
def generate_peptides_from_variants(vars, length, dbadapter, id_type, peptides=None,
    ...
    """
    Generates :class:`~epytope.Core.Peptide.Peptide` from :class:`~epytope.Core.Variant.Variant` and avoids the
    ...
    :param dbadapter: A :class:`~epytope.IO.ADBAdapter.ADBAdapter` to extract relevant transcript information
